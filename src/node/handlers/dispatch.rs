...
impl Node {
    ...
    /// Handle an unexpected transport-level disconnect.
    ///
    /// The transport has already lost the underlying connection, so we tear
    /// down any matching peer or pending handshake state immediately instead of
    /// waiting for heartbeat timeout to notice.
