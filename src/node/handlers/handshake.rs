...
use crate::node::{MAX_COMPETING_MSG1, Node, NodeError};
            ...
            match (existing_epoch, new_epoch) {
                ...
                _ => {
                    ...
                    if competing_msg1_count > MAX_COMPETING_MSG1 {
                        warn!(
                            peer = %self.peer_display_name(&peer_node_addr),
                            count = competing_msg1_count,
                            max = MAX_COMPETING_MSG1,
                            "Dropping duplicate msg1 after competing handshake limit"
                        );
                        self.msg1_rate_limiter.complete_handshake();
                        return;
                    }
