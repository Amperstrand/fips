...
use crate::protocol::{CoordsRequired, PathBroken, SessionAck, SessionDatagram, SessionMsg3};
...
fn forge_setup_from_stranger(nodes: &[TestNode]) -> Vec<u8> {
    use crate::noise::HandshakeState;
    use crate::protocol::SessionSetup;
...
/// Build a node with periodic rekey disabled holding one established session
/// with `peer`, returning the node and the peer's address.
