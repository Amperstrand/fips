                ...
                Ok(ip) => {
                    ...
                    match Self::bind_dns_socket(bind) {
                        Ok(socket) => {
                            ...
                            // Without this, the `::` bind exposes the hosts
                            // file's alias space to any mesh peer. The name
                            // comes from the device the TUN path actually
                            // created, not from the configured one: macOS and
                            // FreeBSD assign utunN/tunN of their own choosing
                            // and the configured name resolves to nothing
                            // there, which left the filter permanently off.
