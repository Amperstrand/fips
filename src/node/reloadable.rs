...
//!   uncovered: a discovery `LookupResponse` writes an entry for a
//!   destination this node may never open a session with. Those entries, and
//!   only those, carry a learn time and are expired at the coordinate cache's
//!   TTL by `purge_expired_path_mtu` on the same tick, which then reseeds any
//!   direct peer whose entry went. Locally derived link MTUs and values
//!   learned inside a session carry no deadline. That sweep is expiry, not a
//!   reload. (The read side could adopt the same lock-free `ArcSwap` shape in
//!   the future, but that is an optimization, not a reload.)
